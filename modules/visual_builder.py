...
def stop_visual_builder(config, signer, compartments, filter_tz, filter_mode):
    target_resources = []
...
def _get_resources(config, signer, compartment_id):
    ...
    summary = oci.pagination.list_call_get_all_results(
        object.list_vb_instances,
        compartment_id
    )

    for inst in summary.data:
        resource = object.get_vb_instance(vb_instance_id=inst.id)
...
def _perform_resource_action(config, signer, resource_id, action):
    ...
    if (action == 'STOP'):    
        stop_response = object.stop_vb_instance(
            resource_id
        )

        response = object.get_vb_instance(
            resource_id
        )
